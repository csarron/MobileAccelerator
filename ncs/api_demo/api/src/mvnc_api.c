...
static mvncStatus load_fw_file(const char *name) {
    ...
    if (fp == NULL) {
        if (mvnc_loglevel) {
            LOGE("mvnc/MvNCAPI.mvcmd not found at: %s\n", mv_cmd_file)
            perror(mv_cmd_file);
        }
        pthread_mutex_unlock(&mm);
    ...
    LOGD("Boot successful, device address %s\n", name);
    return MVNC_OK;
}
...
mvncStatus mvncOpenDevice(const char *name, void **deviceHandle) {
    ...
    pthread_mutex_lock(&mm);
    if (!initialized) {
        LOGD("begin initializing...\n");
        initialize();
    }

    LOGD("begin loading fw cmd file to device...\n");
    rc = load_fw_file(device_name);
    if (rc != MVNC_OK) {
        free(temp);
        LOGE("failed to load fw cmd file!\n");
    }
    ...
    while (time_in_seconds() < waittm) {
        ...
        if (f) {
            ...
            if (!usblink_getmyriadstatus(f, &status) && status == MYRIAD_WAITING) {
                ...
                LOGE("found, but cannot get status\n");
                usblink_close(f);
            }
        }
...
static int deallocate_graph(struct Graph *g) {
    ...
    // Free it with all its test_resources
    if (found) {
        free(g->aux_buffer);
        free(g->output_data);
...
//        for (unsigned int i = 0; i < g->nstages; i++) {
//            LOGD("api ncs layer time: %d, %f ms\n",i, (*(float **) test_resources)[i]);
//        }
            break;
...
mvncStatus mvncSetDeviceOption(void *deviceHandle, int option, const void *data,
    ...
    if (deviceHandle == 0 && option == MVNC_LOG_LEVEL) {
        LOGW("Warning: MVNC_LOG_LEVEL is not a Device Option, please use mvncSetGlobalOption()!\n");
...
mvncStatus mvncGetDeviceOption(void *deviceHandle, int option, void *data,
    ...
    if (deviceHandle == 0 && option == MVNC_LOG_LEVEL) {
        LOGW("Warning: MVNC_LOG_LEVEL is not a Device Option, please use mvncGetGlobalOption()!\n");
